...
def visualize_latent_space3D(
    ...
    context_var="normalized_shape_index",
    ...
    print(f"Use left-click to rotate, right-click+drag to zoom, and middle-click to show 3D mask, Q to end application.")
    # --- 3D Plot ---
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')
    sc = ax.scatter(x, y, z, c=scatter_c, cmap=cmap, s=20, alpha=0.8)
    ...
    if continuous:
        ...
        handles = [
            plt.Line2D([0], [0], marker='o', color='w', label=str(v),
                       markerfacecolor=cmap.colors[i], markersize=8)
            for i, v in enumerate(unique_values)
        ]
        ax.legend(handles=handles, title=context_var, bbox_to_anchor=(1.1, 1), loc='upper left')
